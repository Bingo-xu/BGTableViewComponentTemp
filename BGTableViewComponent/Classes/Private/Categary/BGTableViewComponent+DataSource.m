...
- (void)dealSectionCellDatasWith:(nullable NSArray<BGTableViewRowData*>*)cellDatas section:(NSInteger)section isClear:(BOOL)isClear {
    ...
    BGTableViewSectionData*sectionData = [self.mDataSource objectAtIndex:section];
    if (isClear) {
...
- (void)insertRowDataAtIndex:(NSInteger)index cellData:(nullable BGTableViewRowData*)cellData section:(NSInteger)section {
    ...
    [[self.mDataSource objectAtIndex:section] insertCellDataAtIndex:index cellData:cellData];
    [self reloadData];
}
...
- (void)removeSectionDataModelWith:(NSInteger)section {
    ...
    [self.mDataSource removeObjectAtIndex:section];
    [self reloadData];
}
...
- (void)removeRowDataModelWith:(NSIndexPath*)indexPath {
    ...
    BGTableViewSectionData*sectionData = [self.mDataSource objectAtIndex:indexPath.section];
...
- (BOOL)checkBeyondIndex:(NSInteger)index {
    if (index >= self.mDataSource.count) {
        NSLog(@"YKTableViewComponent数组越界");
        return YES;
    }
    return NO;
}
...
- (void)exchangeSectionDataAtSection:(NSInteger)section1 withObjectAtSection:(NSUInteger)section2 {
    [self.mDataSource exchangeObjectAtIndex:section1 withObjectAtIndex:section2];
    [self reloadData];
}

/**某组内交换顺序*/
- (void)exchangeRowDataAtIndex:(NSInteger)idx1 withObjectAtIndex:(NSUInteger)idx2 section:(NSInteger)section {
    if (section >= self.mDataSource.count) {
...
- (void)addOneCellData:(BGTableViewSectionData*)data {
    ...
    [self.mDataSource addObject:data];
}
...
- (void)registCellWith:(nullable NSString*)cellName bundle:(nullable NSBundle* )bundle {
    if (![self.cellClassArr containsObject:cellName] && cellName) {
        [self.cellClassArr addObject:cellName];
        [self.tableView commonRegisterCellWithClassName:cellName bundle:bundle];
    }
}
